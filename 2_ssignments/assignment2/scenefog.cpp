#include "scenefog.h"

#include <iostream>
using std::endl;
using std::cerr;

#include <glm/gtc/matrix_transform.hpp>
using glm::vec3;
using glm::mat4;
using glm::vec4;

SceneFog::SceneFog() : tPrev(0.0f),
                       plane(50.0f, 50.0f, 1, 1),
                       teapot(14, glm::mat4(1.0f)),
					   sphere(1.0f, 36, 18)
{ }

void SceneFog::initScene()
{
    compileAndLinkShader();
    glEnable(GL_DEPTH_TEST);

	//camera
    view = glm::lookAt(vec3(0.0f,4.0f,6.0f), vec3(0.0f,2.0f,0.0f), vec3(0.0f,1.0f,0.0f));
    projection = mat4(1.0f);
	// view = glm::lookAt(cameraPos, cameraPos + cameraFront, cameraUp);

    angle = 0.0;

    prog.setUniform("Light.L", vec3(0.9f) );
	prog.setUniform("Light.La", vec3(0.2f));
    prog.setUniform("Fog.MaxDist", 30.0f );
    prog.setUniform("Fog.MinDist", 1.0f );
    prog.setUniform("Fog.Color", vec3(0.5f,0.5f,0.5f) );
}

void SceneFog::update( float t )
{
  float deltaT = t - tPrev;
  if(tPrev == 0.0f) deltaT = 0.0f;
  tPrev = t;

  angle += 1.0f * deltaT;
  if (angle > glm::two_pi<float>()) angle -= glm::two_pi<float>();
}

void SceneFog::render()
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    vec4 lightPos = vec4(10.0f * cos(angle), 10.0f, 10.0f * sin(angle), 1.0f);
    prog.setUniform("Light.Position", view * lightPos);

    prog.setUniform("Material.Kd", 0.9f, 0.5f, 0.3f);
    prog.setUniform("Material.Ka", 0.9f * 0.3f, 0.5f * 0.3f, 0.3f * 0.3f);
    prog.setUniform("Material.Ks", 0.0f, 0.0f, 0.0f);
    prog.setUniform("Material.Shininess", 180.0f);
    float dist = 0.0f;
    for( int i = 0 ; i < 4; i++ ) {
        model = mat4(1.0f);
        model = glm::translate(model, vec3(dist * 0.6f - 2.0f,0.0f,-dist));
        model = glm::rotate(model, glm::radians(-90.0f), vec3(1.0f,0.0f,0.0f));
        setMatrices();
        teapot.render();

		model = mat4(1.0f);
		model = glm::translate(model, vec3(dist * 0.6f + 1.5f, 1.75f, -dist+3.0));
		model = glm::rotate(model, glm::radians(-90.0f), vec3(1.0f, 0.0f, 0.0f));
		setMatrices();
		sphere.render();

        dist += 5.0f;
    }

	//prog.setUniform("Material.Kd", 0.9f, 0.5f, 0.3f);
	//prog.setUniform("Material.Ka", 0.9f * 0.3f, 0.5f * 0.3f, 0.3f * 0.3f);
	//prog.setUniform("Material.Ks", 0.0f, 0.0f, 0.0f);
	//prog.setUniform("Material.Shininess", 180.0f);
	//for (int i = 0; i < 4; i++) {
	//	model = mat4(1.0f);
	//	model = glm::translate(model, vec3(dist * 0.6f - 1.0f, 0.75f, -dist+3.0));
	//	model = glm::rotate(model, glm::radians(-90.0f), vec3(1.0f, 0.0f, 0.0f));
	//	setMatrices();
	//	sphere.render();
	//	dist += 3.0f;
	//}

    prog.setUniform("Material.Kd", 0.7f, 0.7f, 0.7f);
    prog.setUniform("Material.Ka", 0.2f, 0.2f, 0.2f);
    prog.setUniform("Material.Ks", 0.0f, 0.0f, 0.0f);
    prog.setUniform("Material.Shininess", 180.0f);

    model = mat4(1.0f);
    setMatrices();
    plane.render();
}

void SceneFog::setMatrices()
{
    mat4 mv = view * model;
    prog.setUniform("ModelViewMatrix", mv);
    prog.setUniform("NormalMatrix",
                    glm::mat3( vec3(mv[0]), vec3(mv[1]), vec3(mv[2]) ));
    prog.setUniform("MVP", projection * mv);
}

void SceneFog::resize(int w, int h)
{
    glViewport(0,0,w,h);
    width = w;
    height = h;
    projection = glm::perspective(glm::radians(60.0f), (float)w/h, 0.3f, 100.0f);
}

void SceneFog::compileAndLinkShader()
{
	try {
		prog.compileShader("shader/fog.vert.glsl");
		prog.compileShader("shader/fog.frag.glsl");
    	prog.link();
    	prog.use();
    } catch(GLSLProgramException & e) {
 		cerr << e.what() << endl;
 		exit( EXIT_FAILURE );
    }
}
