...
void ScenePerFragment::initScene()
{
    ...
    view = glm::lookAt(vec3(3.5f, 3.5f, 5.5f), vec3(0.0f,0.75f,0.0f), vec3(0.0f,1.0f,0.0f));
...
void ScenePerFragment::render()
{
    ...
    prog.setUniform("Light.Position", view * glm::vec4(10.0f * cos(angle), 3.0f, 10.0f * sin(angle), 1.0f) );
...
{
	try {
		//prog.compileShader("shader/perfrag.vert.glsl");
		//prog.compileShader("shader/perfrag.frag.glsl");
		prog.compileShader("shader/blinnphong.vert.glsl");
		prog.compileShader("shader/blinnphong.frag.glsl");
    	prog.link();
    	prog.use();
